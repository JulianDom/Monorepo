...
interface OperativeManagementProps {
  onForceLogout?: (operativeId: string) => void;
}
...
export function OperativeManagement({ onForceLogout }: OperativeManagementProps) {
    ...
    setOperatives(
      operatives.map((op) => {
        if (op.id === operativeId) {
          ...
          // Si se está deshabilitando, forzar logout
          if (newStatus === 'inactive' && onForceLogout) {
            onForceLogout(operativeId);
          }
