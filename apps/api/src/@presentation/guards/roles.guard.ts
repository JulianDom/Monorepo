...
import { ActorType, AuthenticatedActor } from '@shared/types';
 ...
 *
 * Uso:
 * 1. Aplicar @UseGuards(JwtAuthGuard, RolesGuard) al controller o método
 * 2. Usar @Roles(ActorType.ADMIN) para restringir acceso
 *
 * Si no se especifica @Roles(), el guard permite el acceso
 * (asume que solo se requiere autenticación).
 */
@Injectable()
export class RolesGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    // Verificar si el tipo de actor está en los roles permitidos
